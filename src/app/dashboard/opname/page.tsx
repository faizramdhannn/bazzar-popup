...
export default function OpnamePage() {
  ...
  // Focus scan input when session opens
  useEffect(() => {
    if (showSession && sessionPopup && !cameraActive) {
      ...
      try {
        ...
        await codeReader.decodeFromVideoDevice(
          ...
          (result, err) => {
            ...
            const sku = result.getText().trim().toUpperCase();
            setScanError("");

            const found = masterItems.find((item) => item.item_sku === sku);
            if (!found) {
              setScanError(`SKU "${sku}" tidak ditemukan`);
              return;
            }

            const itemName = [found.item_name, found.item_category]
              .filter(Boolean)
              .join(" — ")
              .trim();
            ...
            setSessionItems((prev) => {
              ...
              if (idx >= 0) {
                ...
                updated[idx] = {
                  ...updated[idx],
                  real_scan: updated[idx].real_scan + 1,
                };
        ...
        if (!cancelled) {
          setCameraError("Tidak dapat mengakses kamera: " + String(e));
  ...
  async function handleDeleteItem(opnameId: string, itemSku: string) {
    ...
    await fetch(`/api/opname?opname_id=${opnameId}&item_sku=${itemSku}`, {
      method: "DELETE",
    });
  ...
  const groupColumns: ColumnDef<OpnameGroup, unknown>[] = [
    ...
    {
      header: "Popup",
      accessorKey: "popup_id",
      cell: ({ row }) => getPopupName(row.original.popup_id),
    },
    {
      header: "Tanggal",
      accessorKey: "date",
    ...
    { header: "Oleh", accessorKey: "created_by" },
    {
      ...
      cell: ({ row }) => (
        <div className="flex gap-1">
          <button
            onClick={() => fetchDetail(row.original.opname_id)}
            className="p-1.5 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
          >
            <Eye className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => handleDeleteGroup(row.original.opname_id)}
  ...
  const detailColumns: ColumnDef<OpnameDetail, unknown>[] = [
    {
      ...
      id: "no",
      cell: ({ row }) => (
        <span className="text-zinc-400">{row.index + 1}</span>
      ),
    },
    ...
    { header: "Cutoff", accessorKey: "item_cutoff_qty" },
    {
      ...
      cell: ({ row }) => {
        const diff =
          Number(row.original.item_qty) - Number(row.original.item_cutoff_qty);
        if (diff === 0)
          return <span className="text-zinc-400 tabular-nums">0</span>;
        return (
          <span
        ...
        <div className="flex gap-1">
          <button
            onClick={() => {
              ...
              setEditQty(row.original.item_qty);
            }}
            className="p-1.5 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
          >
          ...
          <button
            onClick={() =>
              handleDeleteItem(row.original.opname_id, row.original.item_sku)
            }
            className="p-1.5 rounded hover:bg-red-50 dark:hover:bg-red-900/20 text-red-500"
          >
            <Trash2 className="w-3.5 h-3.5" />
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              value={sessionPopup}
              onChange={(e) => {
                setSessionPopup(e.target.value);
                setScanError("");
              }}
            ...
            >
              ...
              {popups.map((p) => (
                <option key={p.id_location} value={p.id_location}>
                  {p.popup_name}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                value={scanSku}
                onChange={(e) => {
                  setScanSku(e.target.value.toUpperCase());
                  setScanError("");
                }}
          ...
          <div>
            <button
              ...
              disabled={!sessionPopup}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-colors disabled:opacity-40 ${
                cameraActive
            ...
            >
              ...
              {cameraActive ? "Matikan Kamera" : "Scan Pakai Kamera"}
            </button>
            {cameraError && (
              ...
              <div className="max-h-56 overflow-y-auto">
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-zinc-50 dark:bg-zinc-800/80 border-b border-zinc-200 dark:border-zinc-700">
                    <tr>
                      <th className="px-2 py-1.5 text-left font-medium text-zinc-400 w-7">
                        No.
                      </th>
                      ...
                      <th className="px-2 py-1.5 text-right font-medium text-zinc-400 w-16">
                        Real Scan
                      </th>
                  ...
                  <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                    {sessionItems.length === 0 ? (
                      ...
                      sessionItems.map((r, idx) => (
                        <tr key={r.item_sku}>
                          <td className="px-2 py-1.5 text-zinc-400 text-center">
                            {idx + 1}
                          </td>
                          <td className="px-2 py-1.5 font-mono text-zinc-500 dark:text-zinc-400">
                            {r.item_sku}
                          </td>
                          <td className="px-2 py-1.5 text-zinc-600 dark:text-zinc-300 truncate max-w-[140px]">
                            {r.item_name}
                          </td>
        ...
        <div className="overflow-x-auto rounded-xl border border-zinc-200 dark:border-zinc-800">
          <table className="w-full text-xs md:text-sm min-w-[500px]">
            <thead className="bg-zinc-50 dark:bg-zinc-800/50">
              <tr>
                <th className="px-2 py-1.5 text-left text-xs font-medium text-zinc-400 w-7">
                  No.
                </th>
                <th className="px-2 py-1.5 text-left text-xs font-medium text-zinc-400">
                  Item Code
                </th>
                <th className="px-2 py-1.5 text-left text-xs font-medium text-zinc-400">
                  Item Name
                </th>
                <th className="px-2 py-1.5 text-right text-xs font-medium text-zinc-400">
                  Real Scan
                </th>
                <th className="px-2 py-1.5 text-right text-xs font-medium text-zinc-400">
                  Cutoff
                ...
                <th className="px-2 py-1.5 text-xs font-medium text-zinc-400 w-14">
                  Aksi
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {detail.length === 0 ? (
                ...
                detail.map((row, idx) => {
                  const diff =
                    Number(row.item_qty) - Number(row.item_cutoff_qty);
                  const rowBg =
                    diff === 0
                  ...
                  return (
                    <tr key={row.item_sku} className={rowBg}>
                      <td className="px-2 py-1.5 text-zinc-400 text-center text-xs">
                        {idx + 1}
                      </td>
                      <td className="px-2 py-1.5 font-mono text-xs text-zinc-500 dark:text-zinc-400 whitespace-nowrap">
                        {row.item_sku}
                      </td>
                      <td
                        className="px-2 py-1.5 text-xs text-zinc-700 dark:text-zinc-200 max-w-[160px] truncate"
                        title={row.item_name}
                      >
                        {row.item_name}
                      </td>
                      ...
                      <td className="px-2 py-1.5 text-right text-xs tabular-nums text-zinc-500">
                        {row.item_cutoff_qty}
                      </td>
                      ...
                      <td className="px-2 py-1.5">
                        <div className="flex gap-0.5">
                          <button
                            onClick={() => {
                              setEditRow(row);
                              setEditQty(row.item_qty);
                            }}
                            className="p-1 rounded hover:bg-white/60 dark:hover:bg-zinc-800"
                          >
                            <Pencil className="w-3 h-3" />
                          </button>
                          <button
                            onClick={() =>
                              handleDeleteItem(row.opname_id, row.item_sku)
                            }
